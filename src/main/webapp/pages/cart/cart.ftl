                     ...
                     <#list   productList as pl>
                    	<ul class="clist clear" id="product${pl_index}">
					    	...
					    	<li class="clist_foot">￥<div class="sinprice">${(pl.salesPrice)!"0.00"}</div>
                            	<img src="${rc.contextPath}/pages/images/del.png" class="delcart" />
                            	<div class="cmath fr">
                            		<span class="cmath_ca">-</span>
                            		<span id="currentproduct">${(pl.count)!"0"}</span>
                            		<span class="cmath_c">+</span>
                            		<input type="hidden" id="productId" name="productId" value="${(pl.productId)!"0"}" />
                            		<input type="hidden" id="productstatus" name="productstatus" value="${(pl.status)!"0"}" />
                            		<input type="hidden" id="indexId" name="indexId" value="${pl_index}" />
                            	</div>
                            </li>
...
function cartsubstract(){
	...
	var productId=$(this).parent().find('#productId').val();
	if(productId == "19" && parseInt(tot) <=2){
		//alert("打折商品最低购买两件才能下单哟");
		showMsg('打折商品最低购买两件才能下单哟');
		return false;
	}
	var productcount = parseInt(tot)-1 ;
	var data = "productid="+productId+"&productcount="+productcount ;
