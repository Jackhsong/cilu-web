...
public class WithdrawalsController {
    ...
    public Object getWithdrawals(@RequestParam(value = "cashNum", required = true) float cashNum,
        ...
        QqbsAccountEntity account = SessionUtil.getQqbsAccountFromSession(session);
        try{
        	boolean falg = withdrawalsService.updateDraw(account, cashNum,false);
        	if(!falg){
        		resultMap.put("status", CommonEnum.BUSINESS_RESPONSE_STATUS.FAILED.getValue());
        	}
        	//按时间远近，将可提现的粉丝订单状态改为已提现，未提现完的设为状态仍未2，修改withdrawCash为剩余金额
        	List<QqbsFansOrderEntity> qfoeList = spokesPersonService.getWithdrawcashList(account.getAccountId(),(byte)2);
        	float sum = 0;
        	for(QqbsFansOrderEntity qfoe:qfoeList){
        		if(sum<=cashNum){
        			sum += qfoe.getWithdrawCash();
        			//已提现
        			qfoe.setStatus(3);
        			spokesPersonService.updateStatus(qfoe.getFansOrderId(),qfoe.getFansAccountId(),(byte)qfoe.getStatus());
        			if(sum>=cashNum){
        				if(sum==cashNum){
        					//已提现
        					break;
        				...
        				//部分提现
        				qfoe.setStatus(2);
        				spokesPersonService.updateWithdrawcash(qfoe.getId(), sum-cashNum);
        				spokesPersonService.updateStatus(qfoe.getFansOrderId(),qfoe.getFansAccountId(),(byte)qfoe.getStatus());
        				break;
            		}
        		}
