...
public class HcbBargainController {
	...
	@Resource
	ActivityBargainService activityBargainService;
...
//		return modelAndView;
		return "redirect:/bargain/activity_a_17";
	}
	
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/next_activity_17")
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/activity_a_17")
	public ModelAndView activityA(HttpServletRequest request, HttpServletResponse response, HttpSession session)
		...
		ActivityBargainEntity abe = activityBargainService.findActivityBargainInfoById(2);
		//活动取消，关闭activityA页面链接
		if(abe.getIsAvailable()!=1){
			modelAndView.setViewName("bargain/activityEnd");
			modelAndView.addObject("isAvailable", abe.getIsAvailable());
			return modelAndView;
		}
		...
		//已砍次数
		List<HcbParticipant> participants2 = participantService.listSelectByBargainerUuid(accountId);
		int participantsSum = participants2.size();
		
		if(participantsSum%7==0 && participantsSum > 0){
			if((int)Math.floor(participantsSum/7) == Integer.valueOf(bargain.getIsPay())){
				//此次参与已砍七次，并且已购买
				modelAndView.addObject("currentOver", "2");

				//此次参与帮砍的人数
				modelAndView.addObject("participantsCurrent", participantsSum%7);
			}else{
				//此次参与已砍七次，但是未购买
				modelAndView.addObject("currentOver", "1");
				//此次参与帮砍的人数
				modelAndView.addObject("participantsCurrent", 7);
			}
		}else{
			//此次参与还未砍到7次
			modelAndView.addObject("currentOver", "0");
			//此次参与帮砍的人数
			modelAndView.addObject("participantsCurrent", participantsSum%7);
		}
		
		modelAndView.addObject("participantsSum", participantsSum);
		modelAndView.addObject("accountViewImage", av.getImage());
		modelAndView.addObject("title", abe.getTitle());
		modelAndView.addObject("accountId", accountId);

		

		
		if (bargain == null || bargain.getActivityId() != 17 ) {

			bargain = new HcbBargain();

			bargain.setBargainerUuid(accountId);
			bargain.setCurrentPrice("169.00");
		...
		//砍价超越X%人
		int percent = 100 - (int)((Double.valueOf(currentPrice)/169.00)*100-4);
		
		bargain.setCurrentPrice(String.format("%.2f", currentPrice));

		//当前几折
		Double  currentDiscount=  Double.valueOf(new DecimalFormat("0.00").format(currentPrice/169*10));
		...
		int sum = 0;
		if(bargainService.countTotalPayed() == null){
			sum = 0;
		}else{
			sum = bargainService.countTotalPayed();
		}
		Integer leftNumber = 1000 - sum;
		modelAndView.addObject("leftNumber", String.valueOf(leftNumber));
		modelAndView.addObject("soldNumber", String.valueOf(1000 - leftNumber));
		
		//update activityBargain leftCount
		activityBargainService.updateLeftCount(leftNumber, 2);
		
		// 注入已砍价的人的总数
		/*Integer bargainerNumber = bargainService.countTotalBargained("90.00");
			...
			if (participants != null && participants.size() > 0) {
				...
				for (HcbParticipant participant : participants) {
					...
					if (participant.getUserUuid().equals(String.valueOf(av.getId()))) {
						...
						modelAndView.addObject("isBargained", "1");//判断自己给自己砍
					}
				}
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/activity_b_17")
	public ModelAndView activityB(HttpServletRequest request, HttpServletResponse response, HttpSession session,
		...
		List<HcbParticipant> participants2 = participantService.listSelectByBargainerUuid(accountId);
		int participantsSum = participants2.size();
		int ispay = Integer.parseInt(bargain.getIsPay());
		int times = (int)Math.floor(participantsSum/7);
		...
		//活动发起者的头像
		modelAndView.addObject("accountViewImage", accountEntity.getImage());
		...
		if (accountId == null || accountId.equals("") || accountEntity == null 
			...
			modelAndView.setViewName("bargain/activityA");
		...
		//砍价超越X%人
		int percent = 100 - (int)((Double.valueOf(bargain.getCurrentPrice())/169.00)*100-4);
	...
	public ModelAndView createOrder(HttpServletRequest request, HttpServletResponse response, HttpSession session,
		...
		if (price == null || price.equals("") || Float.valueOf(price) < 1.0 || Float.valueOf(price) > 90.0) {
		...
		ProductCommonEntity pce = pcommondi.findProductCommonInfoById(17);
