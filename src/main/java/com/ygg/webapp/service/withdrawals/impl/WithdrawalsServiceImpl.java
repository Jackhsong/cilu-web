...
public class WithdrawalsServiceImpl implements WithdrawalsService {
	...
	/**
	 * 更新提现金额(tuikuan为true时表示当操作为退款时，总奖励也需要减少)
	 * @return
	 * @throws DaoException
	 * @see com.ygg.webapp.service.withdrawals.WithdrawalsService#getLogList(com.ygg.webapp.entity.QqbsAccountEntity)
	 */
	public boolean updateDraw(QqbsAccountEntity qqbsAccountEntity,float cashNum,boolean tuikuan){
		boolean flag = true;
		...
		if(qqbsReward != null){
			...
			if(withdrawCash<cashNum){
				...
				Map<String, Object> para = new HashMap<String, Object>();
				if(tuikuan){
					para.put("totalReward", qqbsReward.getWithdrawCash() - cashNum);
				}
