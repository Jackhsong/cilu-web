...
public class SpokesPersonServiceImpl implements SpokesPersonService {
	...
	public String getListInfo(QqbsAccountEntity account) {
		...
		float totalDraw = 0;
		float totalwithdrawCash = 0;
		List<QqbsFansOrderEntity> list = hqbsFansOrderDao.getWithdrawcashList(account.getAccountId(),(byte)2);
		for(QqbsFansOrderEntity qfoe:list){
			totalwithdrawCash += qfoe.getWithdrawCash();
			//log.error("status=2    ----->withdrawCash"+qfoe.getWithdrawCash());
		}
		List<QqbsFansOrderEntity> list2 = hqbsFansOrderDao.getWithdrawcashList(account.getAccountId(),(byte)3);
		for(QqbsFansOrderEntity qfoe:list2){
			//可提现金额（可提取的）
			totalDraw += qfoe.getWithdrawCash();
			//log.error("status=3    ----->withdrawCash"+qfoe.getWithdrawCash());
		}
		//累计奖励（可提取的加上已提取的）
		totalDraw = totalDraw+totalwithdrawCash;
		QqbsRewardEntity qqbsReward = hqbsRewardDao.getByAccountIdForUpdate(account.getAccountId());
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("totalReward", totalDraw);
		map.put("id", qqbsReward.getId());
		map.put("withdrawCash", totalwithdrawCash);
		log.error("getListInfo----->withdrawCash:"+totalwithdrawCash+",totalReward:"+totalDraw);
		//更新累计奖励和可提现金额
		hqbsRewardDao.updateHqbsReward(map);
...
//					withdrawCash = (realPrice * 0.2f) * 0.2f;
	...
	@Override
	public List<QqbsFansOrderEntity> getWithdrawcashList(int accountId, byte status) {
		List<QqbsFansOrderEntity> fansOrderList = hqbsFansOrderDao.getWithdrawcashList(accountId,status);
		return fansOrderList;
	}

	@Override
	public int updateStatus(int fansOrderId, int fansAccountId, byte status) {
		return hqbsFansOrderDao.updateStatus(fansOrderId,fansAccountId,status);
	}

	@Override
	public int updateWithdrawcash(int id, float withdrawCash) {
		return hqbsFansOrderDao.updateWithdrawcash(id,withdrawCash);
	}
}
